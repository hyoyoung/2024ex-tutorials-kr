...
**저자:** 제레미 하워드, `fast.ai <https://www.fast.ai>`_. 레이첼 토마스와 프란시스코 잉햄에게 감사드립니다.
"""

###############################################################################
# We recommend running this tutorial as a notebook, not a script. To download the notebook (``.ipynb``) file,
# 이 튜토리얼은 스크립트가 아닌 노트북으로 실행하는 걸 권장드립니다. 노트북('.ipynb') 파일을 다운로드하려면 다음과 같이 하세요,
# click the link at the top of the page.
# 페이지 상단의 링크를 클릭.
# PyTorch provides the elegantly designed modules and classes
# 파이토치는 다음과 같이 우아하게 디자인된 모듈과 클래스들을 제공합니다: 
# `torch.nn <https://pytorch.org/docs/stable/nn.html>`_ ,
# `torch.optim <https://pytorch.org/docs/stable/optim.html>`_ ,
# `Dataset <https://pytorch.org/docs/stable/data.html?highlight=dataset#torch.utils.data.Dataset>`_ ,
# and `DataLoader <https://pytorch.org/docs/stable/data.html?highlight=dataloader#torch.utils.data.DataLoader>`_ 
# to help you create and train neural networks.
# 위의 기능들이 여러분이 신경망을 생성하고 훈련하는 데 도움을 줄 것입니다.
# In order to fully utilize their power and customize them for your problem,
# 이러한 기능을 최대한 활용하고 자신의 문제에 맞게 사용자 정의하기 위해서는
# you need to really understand exactly what they're doing. 
# 신경망이 동작하는 것을 정확히 이해해야 합니다.
...
# 더 잘 이해해보기 위해, 이러한 모델의 기능을 사용하지 않고 먼저 MNIST 데이터 세트에 대해 기본 신경망을 훈련해보겠습니다.
# we will initially only use the most basic PyTorch tensor functionality. 
# 처음에는 가장 기본적인 PyTorch 텐서 기능만 사용합니다. 
# Then, we will incrementally add one feature from ``torch.nn``, ``torch.optim``, ``Dataset``, or
# ``DataLoader`` at a time, showing exactly what each piece does, and how it
# works to make the code either more concise, or more flexible.
# 그런 다음``torch.nn``, ``torch.optim``, ``Dataset`` 또는 "DataLoder에서 한 번에 하나씩 기능을 점진적으로 추가해서
# 각 부분들이 정확히 무엇을 하는지, 코드를 더 간결하게 혹은 더 유연하게끔 만들기 위해 어떻게 작동하는 지를 보여드리겠습니다.
#
# **This tutorial assumes you already have PyTorch installed, and are familiar
# with the basics of tensor operations.** (If you're familiar with Numpy array
# operations, you'll find the PyTorch tensor operations used here nearly identical).
# 이 튜토리얼은 이미 파이토치가 설치되어 있고, 텐서 연산의 기본 사항에 익숙하다고 가정하고 있습니다.
# (넘파이 배열의 연산에 익숙하다면 여기서 사용하는 PyTorch 텐서 연산도 거의 동일합니다.)
#
#
# MNIST data setup
# ----------------
